<?php
    ...
    class WPH_module_general_security_header_content_security_policy_report_only extends WPH_module_component
        {
            ...
            function get_module_component_settings()
                {
                    ...
                    $this->component_settings[]                  =   array(
                                                                    ...
                                                                    'label'         =>  __('Content-Security-Policy-Report-Only',    'wp-hide-security-enhancer'),
                                                                    
                                                                    'help'          =>  array(
                                                                                                'title'                     =>  __('Help',    'wp-hide-security-enhancer') . ' - ' . __('Content-Security-Policy-Report-Only',    'wp-hide-security-enhancer'),
                                                                                                'description'               =>  __("The HTTP Content-Security-Policy-Report-Only response header allows web developers to experiment with Content-Security-Policy policies by monitoring (but not enforcing) their effects. These violation reports consist of JSON documents sent via an HTTP POST request to the specified URI.",    'wp-hide-security-enhancer') .
                                                                                                ...
                                                                                                'option_documentation_url'  =>  'https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy-Report-Only'
                                                                                                ),
            ...
            function _init_content_security_policy_report_only( $saved_field_data )
                {
            ...
            function _callback_saved_content_security_policy_report_only( $saved_field_data )
                {
                                    ...
                                    foreach ( $option   as $item_key    =>  $item )
                                        {
                                            if ( substr( $item_key, - strlen( ':' )) === ':' )
                                                ...
                                                else if ( in_array ( $item_key, array ( 'host', 'nonce / hash' ) ) )
                                                $rewrite_line    .=  " " . $item;
                                                else    
                                                $rewrite_line    .=  " '" . $item_key . "'";
                                        }
                    ...
                    if($this->wph->server_htaccess_config   === TRUE)                               
                        {
                            $rewrite    =  "\n" . '        Header set Content-Security-Policy-Report-Only "' . $rewrite_line . '"';
                    ...
                    if( $this->wph->server_nginx_config   === TRUE )
                        {  
                            ...
                            $rewrite_data  =   '         add_header Content-Security-Policy-Report-Only "' . $rewrite_line . '";';
